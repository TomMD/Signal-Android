...
public class CreateProfileActivity extends BaseActionBarActivity implements InjectableType {
          ...
          new AsyncTask<Void, Void, byte[]>() {
            ...
            protected void onPostExecute(byte[] result) {
              if (result != null) {
                ...
                Toast.makeText(
                        CreateProfileActivity.this,
                        R.string.CreateProfileActivity_error_setting_profile_photo,
                        Toast.LENGTH_LONG)
                    .show();
              }
            }
  ...
  private void initializeResources() {
    ...
    this.finishButton.setOnClickListener(
        view -> {
    ...
    informationLabel.setOnClickListener(
        view -> {
  ...
  private void initializeProfileName(boolean excludeSystem) {
    if (!TextUtils.isEmpty(TextSecurePreferences.getProfileName(this))) {
      ...
      SystemProfileUtil.getSystemProfileName(this)
          .addListener(
              new ListenableFuture.Listener<String>() {
                @Override
  ...
  private void initializeProfileAvatar(boolean excludeSystem) {
    ...
    if (AvatarHelper.getAvatarFile(this, ourAddress).exists()
        && AvatarHelper.getAvatarFile(this, ourAddress).length() > 0) {
      new AsyncTask<Void, Void, byte[]>() {
        @Override
        protected byte[] doInBackground(Void... params) {
          try {
            return Util.readFully(
                AvatarHelper.getInputStreamFor(CreateProfileActivity.this, ourAddress));
          } catch (IOException e) {
            Log.w(TAG, e);
            return null;
          }
      ...
      SystemProfileUtil.getSystemProfileAvatar(this, new ProfileMediaConstraints())
          .addListener(
              new ListenableFuture.Listener<byte[]>() {
                @Override
  ...
  private void initializeEmojiInput() {
    ...
    this.emojiToggle.setOnClickListener(
        v -> {
          if (container.getCurrentInput() == emojiDrawer) {
    ...
    this.emojiDrawer.setEmojiEventListener(
        new EmojiDrawer.EmojiEventListener() {
          @Override
  ...
  private Intent createAvatarSelectionIntent(
      @Nullable File captureFile, boolean includeClear, boolean includeCamera) {
    ...
    Intent chooserIntent =
        Intent.createChooser(
            galleryIntent, getString(R.string.CreateProfileActivity_profile_photo));
  ...
  private void handleAvatarSelectionWithPermissions() {
    ...
    Intent chooserIntent =
        createAvatarSelectionIntent(captureFile, avatarBytes != null, hasCameraPermission);
  ...
  private void handleUpload() {
    ...
    if (avatarBytes == null || avatarBytes.length == 0) avatar = null;
      ...
      avatar =
          new StreamDetails(
              new ByteArrayInputStream(avatarBytes), "image/jpeg", avatarBytes.length);
    ...
    new AsyncTask<Void, Void, Boolean>() {
      ...
      protected Boolean doInBackground(Void... params) {
        ...
        try {
          ...
          TextSecurePreferences.setProfileAvatarId(
              CreateProfileActivity.this, new SecureRandom().nextInt());
        } catch (IOException e) {
          Log.w(TAG, e);
          return false;
        }
      ...
      public void onPostExecute(Boolean result) {
        ...
        if (result) {
          ...
          Toast.makeText(
                  CreateProfileActivity.this,
                  R.string.CreateProfileActivity_problem_setting_profile,
                  Toast.LENGTH_LONG)
              .show();
        }
      }
  ...
  private void handleFinishedLollipop() {
    ...
    Animator animation =
        ViewAnimationUtils.createCircularReveal(
            reveal, finishX, finishY, 0f, (float) Math.max(reveal.getWidth(), reveal.getHeight()));
