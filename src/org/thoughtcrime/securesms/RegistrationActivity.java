...
/**
 * The register account activity. Prompts ths user for their registration information and begins the
 * account registration process.
 *
 * @author Moxie Marlinspike
 */
public class RegistrationActivity extends BaseActionBarActivity
    implements VerificationCodeView.OnCodeEnteredListener {
  ...
  private void initializeResources() {
    ...
    this.registrationState =
        new RegistrationState(RegistrationState.State.INITIAL, null, null, null);
    ...
    this.keyboard.setOnKeyPressListener(
        key -> {
  ...
  private void initializeSpinner() {
    ...
    this.countrySpinnerAdapter.setDropDownViewResource(
        android.R.layout.simple_spinner_dropdown_item);

    setCountryDisplay(getString(R.string.RegistrationActivity_select_your_country));

    this.countrySpinner.setAdapter(this.countrySpinnerAdapter);
    this.countrySpinner.setOnTouchListener(
        (v, event) -> {
    ...
    this.countrySpinner.setOnKeyListener(
        (v, keyCode, event) -> {
    ...
    if (localNumber.isPresent()) {
      ...
      if (simCountryIso.isPresent() && !TextUtils.isEmpty(simCountryIso.get())) {
        this.countryCode.setText(
            String.valueOf(
                PhoneNumberUtil.getInstance().getCountryCodeForRegion(simCountryIso.get())));
      }
    }
  }
  ...
  private void initializePermissions() {
    Permissions.with(RegistrationActivity.this)
        ...
        .withRationaleDialog(
            getString(
                R.string
                    .RegistrationActivity_signal_needs_access_to_your_contacts_and_media_in_order_to_connect_with_friends),
  ...
  private String getConfiguredE164Number() {
    return PhoneNumberFormatter.formatE164(
        countryCode.getText().toString(), number.getText().toString());
  }
  ...
  private void handleRestore(BackupUtil.BackupInfo backup) {
    ...
    new AlertDialog.Builder(this)
        ...
        .setView(view)
        .setPositiveButton(
            getString(R.string.RegistrationActivity_restore),
            (dialog, which) -> {
                ...
                protected void onPostExecute(@NonNull Boolean result) {
                  ...
                  if (result) {
                    ...
                    Toast.makeText(
                            ...
                            R.string.RegistrationActivity_incorrect_backup_passphrase,
                            Toast.LENGTH_LONG)
                        .show();
                  }
                }
  ...
  private void handleRegister() {
    ...
    Permissions.with(this)
        ...
        .withRationaleDialog(
            getString(
                R.string
                    .RegistrationActivity_to_easily_verify_your_phone_number_signal_can_automatically_detect_your_verification_code),
    ...
    new AsyncTask<Void, Void, Pair<String, Optional<String>>>() {
      ...
      protected @Nullable Pair<String, Optional<String>> doInBackground(Void... voids) {
        try {
          ...
          if (gcmSupported) {
            gcmToken =
                Optional.of(
                    GoogleCloudMessaging.getInstance(RegistrationActivity.this)
                        .register(GcmRefreshJob.REGISTRATION_ID));
          } else {
            gcmToken = Optional.absent();
          }

          accountManager =
              AccountManagerFactory.createManager(RegistrationActivity.this, e164number, password);
      ...
      protected void onPostExecute(@Nullable Pair<String, Optional<String>> result) {
        if (result == null) {
          Toast.makeText(
                  RegistrationActivity.this,
                  R.string.RegistrationActivity_unable_to_connect_to_service,
                  Toast.LENGTH_LONG)
              .show();
          return;
        }
        ...
        registrationState =
            new RegistrationState(
                RegistrationState.State.VERIFYING, e164number, result.first, result.second);
  ...
  private void handleChallengeReceived(@Nullable String challenge) {
    if (challenge != null
        && challenge.length() == 6
        && registrationState.state == RegistrationState.State.VERIFYING) {
      ...
      try {
        for (int i = 0; i < challenge.length(); i++) {
          ...
          verificationCodeView.postDelayed(
              () ->
                  verificationCodeView.append(
                      Integer.parseInt(Character.toString(challenge.charAt(index)))),
              i * 200);
        }
  ...
  public void onCodeComplete(@NonNull String code) {
    this.registrationState =
        new RegistrationState(RegistrationState.State.CHECKING, this.registrationState);
              ...
              .addListener(
                  new AssertedSuccessListener<Boolean>() {
                    ...
                    public void onSuccess(Boolean r) {
                      registrationState =
                          new RegistrationState(RegistrationState.State.PIN, registrationState);
              ...
              .addListener(
                  new AssertedSuccessListener<Boolean>() {
                    ...
                    public void onSuccess(Boolean result) {
                      registrationState =
                          new RegistrationState(
                              RegistrationState.State.VERIFYING, registrationState);
  ...
  private void handleVerifyWithPinClicked(@NonNull String code, @Nullable String pin) {
    if (TextUtils.isEmpty(pin) || TextUtils.isEmpty(pin.replace(" ", ""))) {
      Toast.makeText(
              this,
              R.string.RegistrationActivity_you_must_enter_your_registration_lock_PIN,
              Toast.LENGTH_LONG)
          .show();
      return;
    }
    ...
    new AsyncTask<Void, Void, Integer>() {
      ...
      protected void onPostExecute(Integer result) {
        ...
        if (result == 1) {
          ...
          TextSecurePreferences.setRegistrationLockLastReminderTime(
              RegistrationActivity.this, System.currentTimeMillis());
          TextSecurePreferences.setRegistrationLockNextReminderInterval(
              RegistrationActivity.this, RegistrationLockReminders.INITIAL_INTERVAL);
          ...
          Toast.makeText(
                  RegistrationActivity.this,
                  R.string.RegistrationActivity_incorrect_registration_lock_pin,
                  Toast.LENGTH_LONG)
              .show();
        } else if (result == 3) {
          new AlertDialog.Builder(RegistrationActivity.this)
              ...
              .setMessage(
                  R.string
                      .RegistrationActivity_you_have_made_too_many_incorrect_registration_lock_pin_attempts_please_try_again_in_a_day)
  ...
  private void handleForgottenPin(long timeRemaining) {
    new AlertDialog.Builder(RegistrationActivity.this)
        ...
        .setMessage(
            getString(
                R.string
                    .RegistrationActivity_registration_of_this_phone_number_will_be_possible_without_your_registration_lock_pin_after_seven_days_have_passed,
                (TimeUnit.MILLISECONDS.toDays(timeRemaining) + 1)))
        .setPositiveButton(android.R.string.ok, null)
        .show();
  }
  ...
  private void verifyAccount(@NonNull String code, @Nullable String pin) throws IOException {
    ...
    accountManager.verifyAccountWithCode(
        code, signalingKey, registrationId, !registrationState.gcmToken.isPresent(), pin);
    ...
    SignedPreKeyRecord signedPreKey =
        PreKeyUtil.generateSignedPreKey(RegistrationActivity.this, identityKey, true);
    ...
    TextSecurePreferences.setGcmRegistrationId(
        RegistrationActivity.this, registrationState.gcmToken.orNull());
    TextSecurePreferences.setGcmDisabled(
        RegistrationActivity.this, !registrationState.gcmToken.isPresent());
    ...
    DatabaseFactory.getIdentityDatabase(RegistrationActivity.this)
        .saveIdentity(
            Address.fromSerialized(registrationState.e164number),
    ...
    TextSecurePreferences.setPushServerPassword(
        RegistrationActivity.this, registrationState.password);
  ...
  private void handleSuccessfulRegistration() {
    ApplicationContext.getInstance(RegistrationActivity.this)
        .getJobManager()
        .add(new DirectoryRefreshJob(RegistrationActivity.this, false));
  ...
  private void displayRestoreView(@NonNull BackupUtil.BackupInfo backup) {
    title
        .animate()
        .translationX(title.getWidth())
        .setDuration(SCENE_TRANSITION_DURATION)
        .setListener(
    ...
    subtitle
        .animate()
        .translationX(subtitle.getWidth())
        .setDuration(SCENE_TRANSITION_DURATION)
        .setListener(
            new AnimationCompleteListener() {
              @Override
              public void onAnimationEnd(Animator animation) {
                subtitle.setText(
                    R.string
                        .RegistrationActivity_restore_your_messages_and_media_from_a_local_backup);
    ...
    registrationContainer
        .animate()
        .translationX(registrationContainer.getWidth())
        .setDuration(SCENE_TRANSITION_DURATION)
        .setListener(
            new AnimationCompleteListener() {
              @Override
              public void onAnimationEnd(Animator animation) {
                ...
                restoreBackupSize.setText(
                    getString(
                        R.string.RegistrationActivity_backup_size_s,
                        Util.getPrettyFileSize(backup.getSize())));
                restoreBackupTime.setText(
                    getString(
                        R.string.RegistrationActivity_backup_timestamp_s,
                        DateUtils.getExtendedRelativeTimeSpanString(
                            RegistrationActivity.this, Locale.US, backup.getTimestamp())));
                restoreBackupProgress.setText("");
                restoreContainer
                    .animate()
                    .translationX(0)
  ...
  private void displayInitialView(boolean forwards) {
    ...
    title
        .animate()
        .translationX(startDirectionMultiplier * title.getWidth())
        .setDuration(SCENE_TRANSITION_DURATION)
        .setListener(
            new AnimationCompleteListener() {
              @Override
    ...
    subtitle
        .animate()
        .translationX(startDirectionMultiplier * subtitle.getWidth())
        .setDuration(SCENE_TRANSITION_DURATION)
        .setListener(
            new AnimationCompleteListener() {
              @Override
    ...
    container
        .animate()
        .translationX(startDirectionMultiplier * container.getWidth())
        .setDuration(SCENE_TRANSITION_DURATION)
        .setListener(
            new AnimationCompleteListener() {
              @Override
              public void onAnimationEnd(Animator animation) {
                ...
                registrationContainer.setTranslationX(
                    endDirectionMultiplier * registrationContainer.getWidth());
    ...
    fab.animate()
        .rotationBy(startDirectionMultiplier * 360f)
        .setDuration(SCENE_TRANSITION_DURATION)
        .setListener(
            new AnimationCompleteListener() {
              @Override
              public void onAnimationEnd(Animator animation) {
                ...
                fab.animate()
                    .rotationBy(startDirectionMultiplier * 375f)
                    .setDuration(SCENE_TRANSITION_DURATION)
                    .setListener(null)
                    .start();
              }
  ...
  private void displayVerificationView(@NonNull String e164number, int callCountdown) {
    ...
    subtitle
        .animate()
        .translationX(-1 * subtitle.getWidth())
        .setDuration(SCENE_TRANSITION_DURATION)
        .setListener(
            new AnimationCompleteListener() {
              @Override
              public void onAnimationEnd(Animator animation) {
                SpannableString subtitleDescription =
                    new SpannableString(
                        getString(
                            R.string
                                .RegistrationActivity_please_enter_the_verification_code_sent_to_s,
                            e164number));
                SpannableString wrongNumber =
                    new SpannableString(getString(R.string.RegistrationActivity_wrong_number));
                ...
                ClickableSpan clickableSpan =
                    new ClickableSpan() {
                      ...
                      public void onClick(View widget) {
                        ...
                        registrationState =
                            new RegistrationState(
                                RegistrationState.State.INITIAL, null, null, null);
                      }
                ...
                wrongNumber.setSpan(
                    clickableSpan, 0, wrongNumber.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);

                subtitle.setText(
                    new SpannableStringBuilder(subtitleDescription)
                        .append(" ")
                        .append(wrongNumber));
    ...
    registrationContainer
        .animate()
        .translationX(-1 * registrationContainer.getWidth())
        .setDuration(SCENE_TRANSITION_DURATION)
        .setListener(
            new AnimationCompleteListener() {
              @Override
  ...
  private void displayPinView(String code, long lockedUntil) {
    ...
    verificationContainer
        .animate()
        .translationX(-1 * verificationContainer.getWidth())
        .setDuration(SCENE_TRANSITION_DURATION)
        .setListener(
            new AnimationCompleteListener() {
              @Override
  ...
  private void handlePromptForNoPlayServices(@NonNull String e164number) {
    ...
    dialog.setPositiveButton(
        R.string.RegistrationActivity_i_understand,
        (dialog1, which) -> handleRequestVerification(e164number, false));
  ...
  public void onEvent(FullBackupBase.BackupEvent event) {
    if (event.getCount() == 0)
      ...
      restoreBackupProgress.setText(
          getString(R.string.RegistrationActivity_d_messages_so_far, event.getCount()));
  }
  ...
  private class NumberChangedListener implements TextWatcher {
    ...
    @Override
    public void afterTextChanged(Editable s) {
      if (countryFormatter == null) return;
