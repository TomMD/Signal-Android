...
public class SmsDatabase extends MessagingDatabase {
  ...
  public static final String[] CREATE_INDEXS = {
    ...
    "CREATE INDEX IF NOT EXISTS sms_read_and_notified_and_thread_id_index ON "
        + TABLE_NAME
        + "("
        + READ
        + ","
    ...
    "CREATE INDEX IF NOT EXISTS sms_thread_date_index ON "
        + TABLE_NAME
        + " ("
        + THREAD_ID
        + ", "
  ...
  public void markAsPreKeyBundle(long id) {
    updateTypeBitmask(
        id, Types.KEY_EXCHANGE_MASK, Types.KEY_EXCHANGE_BIT | Types.KEY_EXCHANGE_BUNDLE_BIT);
  }
  ...
  public void markAsSent(long id, boolean isSecure) {
    updateTypeBitmask(
        ...
        Types.BASE_SENT_TYPE | (isSecure ? Types.PUSH_MESSAGE_BIT | Types.SECURE_MESSAGE_BIT : 0));
  }
  ...
  public void incrementReceiptCount(
      SyncMessageId messageId, boolean deliveryReceipt, boolean readReceipt) {
    SQLiteDatabase database = databaseHelper.getWritableDatabase();
    Cursor cursor = null;
  ...
  public List<Pair<Long, Long>> setTimestampRead(SyncMessageId messageId, long expireStarted) {
    ...
    try {
      cursor =
          database.query(
              TABLE_NAME,
              new String[] {ID, THREAD_ID, ADDRESS, TYPE, EXPIRES_IN},
              DATE_SENT + " = ?",
              new String[] {String.valueOf(messageId.getTimetamp())},
              null,
              null,
  ...
  private List<MarkedMessageInfo> setMessagesRead(String where, String[] arguments) {
    ...
    try {
      cursor =
          database.query(
              TABLE_NAME,
              new String[] {ID, ADDRESS, DATE_SENT, TYPE, EXPIRES_IN, EXPIRE_STARTED},
              where,
              arguments,
              null,
              null,
      ...
      while (cursor != null && cursor.moveToNext()) {
        if (Types.isSecureType(cursor.getLong(3))) {
          ...
          ExpirationInfo expirationInfo =
              new ExpirationInfo(cursor.getLong(0), cursor.getLong(4), cursor.getLong(5), false);
  ...
  private @NonNull Pair<Long, Long> insertCallLog(
      @NonNull Address address, long type, boolean unread) {
    ...
    if (message.isPush() && isDuplicate(message, threadId)) {
      ...
      if (message.getSubscriptionId() != -1) {
        DatabaseFactory.getRecipientDatabase(context)
            .setDefaultSubscriptionId(recipient, message.getSubscriptionId());
      }
  ...
  public Cursor getMessageCursor(long messageId) {
    SQLiteDatabase db = databaseHelper.getReadableDatabase();
    return db.query(
        TABLE_NAME, MESSAGE_PROJECTION, ID_WHERE, new String[] {messageId + ""}, null, null, null);
  }
  ...
  private boolean isDuplicate(IncomingTextMessage message, long threadId) {
    ...
    Cursor cursor =
        database.query(
            ...
            new String[] {
              String.valueOf(message.getSentTimestampMillis()),
              message.getSender().serialize(),
  ...
  public class Reader {
    ...
    public SmsMessageRecord getCurrent() {
      ...
      int deliveryReceiptCount =
          cursor.getInt(cursor.getColumnIndexOrThrow(SmsDatabase.DELIVERY_RECEIPT_COUNT));
      int readReceiptCount =
          cursor.getInt(cursor.getColumnIndexOrThrow(SmsDatabase.READ_RECEIPT_COUNT));
