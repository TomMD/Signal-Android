...
public class SQLCipherMigrationHelper {
  ...
  static void migratePlaintext(
    ...
    try {
      GenericForegroundService.startForegroundTask(
          context, context.getString(R.string.SQLCipherMigrationHelper_migrating_signal_database));
  ...
  public static void migrateCiphertext(
      @NonNull Context context,
    ...
    AsymmetricMasterCipher legacyAsymmetricCipher =
        new AsymmetricMasterCipher(
            MasterSecretUtil.getAsymmetricMasterSecret(context, masterSecret));
    ...
    try {
      ...
      copyTable(
          ...
          modernDb,
          (row, progress) -> {
            Pair<Long, String> plaintext =
                getPlaintextBody(
                    legacyCipher,
                    legacyAsymmetricCipher,
                    row.getAsLong("type"),
            ...
            try {
              if (!TextUtils.isEmpty(mediaKey)) {
                ...
                if (mediaKey.startsWith("?ASYNC-")) {
                  plaintext =
                      legacyAsymmetricCipher.decryptBytes(
                          Base64.decode(mediaKey.substring("?ASYNC-".length())));
                } else {
      ...
      copyTable(
          ...
          (row, progress) -> {
            ...
            Pair<Long, String> plaintext =
                getPlaintextBody(
                    legacyCipher, legacyAsymmetricCipher, snippetType, row.getAsString("snippet"));
      ...
      AttachmentSecretProvider.getInstance(context)
          .setClassicKey(
              context,
  ...
  private static Pair<Long, String> getPlaintextBody(
      @NonNull MasterCipher legacyCipher,
  ...
  private static Set<String> getTableColumns(
      String tableName, net.sqlcipher.database.SQLiteDatabase database) {
