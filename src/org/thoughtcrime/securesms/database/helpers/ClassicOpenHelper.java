...
public class ClassicOpenHelper extends SQLiteOpenHelper {
  ...
  public void onApplicationLevelUpgrade(
          ...
          try {
            String body =
                masterCipher.decryptBody(
                    smsCursor.getString(smsCursor.getColumnIndexOrThrow("body")));
      ...
      do {
        ...
        while (threadCursor != null && threadCursor.moveToNext()) {
          ...
          try {
            ...
            long snippetType =
                threadCursor.getLong(threadCursor.getColumnIndexOrThrow("snippet_type"));
    ...
    if (fromVersion < DatabaseUpgradeActivity.MMS_BODY_VERSION) {
      ...
      Log.w(
          "DatabaseFactory",
          "Got MMS rows: " + (mmsCursor == null ? "null" : mmsCursor.getCount()));
      ...
      while (mmsCursor != null && mmsCursor.moveToNext()) {
        ...
        Cursor partCursor =
            db.query(
                "part",
                new String[] {"_id", "ct", "_data", "encrypted"},
                "mid = ?",
                new String[] {mmsId + ""},
                null,
                null,
        ...
        while (partCursor != null && partCursor.moveToNext()) {
          ...
          if (MediaUtil.isTextType(contentType)) {
            try {
              ...
              boolean encrypted =
                  partCursor.getInt(partCursor.getColumnIndexOrThrow("encrypted")) == 1;
              File dataFile = new File(dataLocation);

              InputStream is;

              AttachmentSecret attachmentSecret =
                  new AttachmentSecret(
              ...
              body =
                  (body == null)
                      ? Util.readFullyAsString(is)
                      : body + " " + Util.readFullyAsString(is);
        ...
        if (sessions != null) {
          for (File session : sessions) {
            ...
            if (name.matches("[0-9]+")) {
              ...
              // already upgraded.
              //              IdentityKey identityKey     = Session.getRemoteIdentityKey(context,
              // masterSecret, recipientId);

              if (identityKey != null) {
                MasterCipher masterCipher = new MasterCipher(masterSecret);
                String identityKeyString = Base64.encodeBytes(identityKey.serialize());
                String macString =
                    Base64.encodeBytes(masterCipher.getMacFor(recipientId + identityKeyString));

                db.execSQL(
                    "REPLACE INTO identities (recipient, key, mac) VALUES (?, ?, ?)",
      ...
      if (!MasterSecretUtil.hasAsymmericMasterSecret(context)) {
        ...
        try {
          ...
          while (cursor.moveToNext()) {
            ...
            update.put(
                SmsDatabase.TYPE,
                type | 0x80000000); // Inline now deprecated symmetric encryption type
  ...
  public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
    ...
    if (oldVersion < INTRODUCED_INDEXES_VERSION) {
      ...
      executeStatements(
          db, new String[] {"CREATE INDEX IF NOT EXISTS part_mms_id_index ON part (mid);"});
      executeStatements(
          db,
          new String[] {
    ...
    if (oldVersion < INTRODUCED_NEW_TYPES_VERSION) {
      ...
      db.execSQL(
          ...
          new String[] {(21L | 0x800000L) + "", 42L + ""});
      db.execSQL(
          "UPDATE sms SET type = ? WHERE type = ?",
          new String[] {(23L | 0x800000L) + "", 44L + ""});
      db.execSQL(
          "UPDATE sms SET type = ? WHERE type = ?",
          new String[] {(20L | 0x800000L) + "", 45L + ""});
      db.execSQL(
          "UPDATE sms SET type = ? WHERE type = ?",
          new String[] {(20L | 0x800000L | 0x10000000L) + "", 46L + ""});
      db.execSQL("UPDATE sms SET type = ? WHERE type = ?", new String[] {(20L) + "", 47L + ""});
      db.execSQL(
          "UPDATE sms SET type = ? WHERE type = ?",
          new String[] {(20L | 0x800000L | 0x08000000L) + "", 48L + ""});

      db.execSQL(
          "UPDATE sms SET body = substr(body, ?), type = type | ? WHERE body LIKE ?",
          new String[] {
            (SYMMETRIC_ENCRYPT.length() + 1) + "", 0x80000000L + "", SYMMETRIC_ENCRYPT + "%"
          });

      db.execSQL(
          "UPDATE sms SET body = substr(body, ?), type = type | ? WHERE body LIKE ?",
          new String[] {
            (ASYMMETRIC_LOCAL_ENCRYPT.length() + 1) + "",
            0x40000000L + "",
      ...
      db.execSQL(
          "UPDATE sms SET body = substr(body, ?), type = type | ? WHERE body LIKE ?",
          new String[] {(KEY_EXCHANGE.length() + 1) + "", 0x8000L + "", KEY_EXCHANGE + "%"});

      db.execSQL(
          "UPDATE sms SET body = substr(body, ?), type = type | ? WHERE body LIKE ?",
          new String[] {
            (PROCESSED_KEY_EXCHANGE.length() + 1) + "",
            (0x8000L | 0x2000L) + "",
      ...
      db.execSQL(
          ...
          new String[] {(20L | 0x80000000L) + "", 1 + ""});
      db.execSQL(
          "UPDATE mms SET msg_box = ? WHERE msg_box = ?",
          new String[] {(23L | 0x80000000L) + "", 2 + ""});
      db.execSQL(
          "UPDATE mms SET msg_box = ? WHERE msg_box = ?",
          new String[] {(21L | 0x80000000L) + "", 4 + ""});
      db.execSQL(
          "UPDATE mms SET msg_box = ? WHERE msg_box = ?",
          new String[] {(24L | 0x80000000L) + "", 12 + ""});

      db.execSQL(
          "UPDATE mms SET msg_box = ? WHERE msg_box = ?",
          new String[] {(21L | 0x80000000L | 0x800000L) + "", 5 + ""});
      db.execSQL(
          "UPDATE mms SET msg_box = ? WHERE msg_box = ?",
          new String[] {(23L | 0x80000000L | 0x800000L) + "", 6 + ""});
      db.execSQL(
          "UPDATE mms SET msg_box = ? WHERE msg_box = ?",
          new String[] {(20L | 0x20000000L | 0x800000L) + "", 7 + ""});
      db.execSQL(
          "UPDATE mms SET msg_box = ? WHERE msg_box = ?",
          new String[] {(20L | 0x80000000L | 0x800000L) + "", 8 + ""});
      db.execSQL(
          "UPDATE mms SET msg_box = ? WHERE msg_box = ?",
          new String[] {(20L | 0x08000000L | 0x800000L) + "", 9 + ""});
      db.execSQL(
          "UPDATE mms SET msg_box = ? WHERE msg_box = ?",
          new String[] {(20L | 0x10000000L | 0x800000L) + "", 10 + ""});
      ...
      db.execSQL(
          "UPDATE thread SET snippet = substr(snippet, ?), "
    ...
    if (oldVersion < INTRODUCED_MMS_FROM_VERSION) {
      ...
      Cursor cursor =
          db.query("mms_addresses", null, "type = ?", new String[] {0x89 + ""}, null, null, null);
      ...
      while (cursor != null && cursor.moveToNext()) {
        ...
        if (!TextUtils.isEmpty(address)) {
          db.execSQL(
              "UPDATE mms SET address = ? WHERE _id = ?", new String[] {address, mmsId + ""});
        }
      }
    ...
    if (oldVersion < MIGRATED_CONVERSATION_LIST_STATUS_VERSION) {
      ...
      while (threadCursor != null && threadCursor.moveToNext()) {
        ...
        Cursor cursor =
            db.rawQuery(
                "SELECT DISTINCT date AS date_received, status, "
    ...
    if (oldVersion < NO_MORE_CANONICAL_ADDRESS_DATABASE) {
      SQLiteOpenHelper canonicalAddressDatabaseHelper =
          new SQLiteOpenHelper(context, "canonical_address.db", null, 1) {
            @Override
            public void onCreate(SQLiteDatabase db) {
      ...
      SQLiteDatabase canonicalAddressDatabase =
          canonicalAddressDatabaseHelper.getReadableDatabase();
      NumberMigrator numberMigrator =
          new NumberMigrator(TextSecurePreferences.getLocalNumber(context));

      // Migrate Thread Database
      Cursor cursor =
          db.query("thread", new String[] {"_id", "recipient_ids"}, null, null, null, null, null);
      ...
      while (cursor != null && cursor.moveToNext()) {
        ...
        for (int i = 0; i < recipientIds.length; i++) {
          Cursor resolved =
              canonicalAddressDatabase.query(
                  "canonical_addresses",
                  new String[] {"address"},
                  "_id = ?",
                  new String[] {recipientIds[i]},
                  null,
                  null,
                  null);
      ...
      cursor =
          db.query("mms_addresses", new String[] {"_id", "address"}, null, null, null, null, null);
      ...
      cursor =
          db.query(
              "sms",
              new String[] {"_id", "mismatched_identities"},
      ...
      while (cursor != null && cursor.moveToNext()) {
        ...
        if (!TextUtils.isEmpty(document)) {
          try {
            PreCanonicalAddressIdentityMismatchList oldDocumentList =
                JsonUtils.fromJson(document, PreCanonicalAddressIdentityMismatchList.class);
            ...
            for (PreCanonicalAddressIdentityMismatchDocument oldDocument : oldDocumentList.list) {
              Cursor resolved =
                  canonicalAddressDatabase.query(
                      ...
                      new String[] {"address"},
                      "_id = ?",
                      new String[] {String.valueOf(oldDocument.recipientId)},
                      null,
                      null,
                      null);
              ...
              if (resolved != null && resolved.moveToFirst()) {
                String address = resolved.getString(0);
                newDocumentList.add(
                    new PostCanonicalAddressIdentityMismatchDocument(
                        numberMigrator.migrate(address), oldDocument.identityKey));
              } else {
            ...
            values.put(
                ...
                JsonUtils.toJson(new PostCanonicalAddressIdentityMismatchList(newDocumentList)));
            db.update("sms", values, "_id = ?", new String[] {String.valueOf(id)});
          } catch (IOException e) {
            Log.w(TAG, e);
          }
      ...
      cursor =
          db.query(
              "mms",
              new String[] {"_id", "network_failures"},
      ...
      while (cursor != null && cursor.moveToNext()) {
        ...
        if (!TextUtils.isEmpty(document)) {
          try {
            PreCanonicalAddressNetworkFailureList oldDocumentList =
                JsonUtils.fromJson(document, PreCanonicalAddressNetworkFailureList.class);
            ...
            for (PreCanonicalAddressNetworkFailureDocument oldDocument : oldDocumentList.list) {
              ...
              if (resolved != null && resolved.moveToFirst()) {
                String address = resolved.getString(0);
                newDocumentList.add(
                    new PostCanonicalAddressNetworkFailureDocument(
                        numberMigrator.migrate(address)));
              } else {
                throw new AssertionError("Unable to resolve: " + oldDocument.recipientId);
              }
            ...
            values.put(
                "network_failures",
                JsonUtils.toJson(new PostCanonicalAddressNetworkFailureList(newDocumentList)));
            db.update("mms", values, "_id = ?", new String[] {String.valueOf(id)});
          } catch (IOException e) {
            Log.w(TAG, e);
          }
        ...
        for (File session : sessions) {
          try {
            ...
            if (resolved != null && resolved.moveToNext()) {
              ...
              File destination =
                  new File(
                      session.getParentFile(), address + (deviceId != 1 ? "." + deviceId : ""));
    ...
    if (oldVersion < NO_MORE_RECIPIENTS_PLURAL) {
      ...
      while (cursor != null && cursor.moveToNext()) {
        ...
        if (addressList.length == 1) {
          ...
          contentValues.put("recipient_ids", DelimiterUtil.unescape(addressListString, ' '));
          db.update(
              "recipient_preferences", contentValues, "_id = ?", new String[] {String.valueOf(id)});
        } else {
          Log.w(
              TAG,
              "Found preferences for MMS thread that appears to be gone: " + addressListString);
          db.delete("recipient_preferences", "_id = ?", new String[] {String.valueOf(id)});
        }
      }
    ...
    if (oldVersion < INTERNAL_DIRECTORY) {
      ...
      Cursor cursor =
          directoryDatabase.query(
              "directory", new String[] {"number", "registered"}, null, null, null, null, null);

      while (cursor != null && cursor.moveToNext()) {
        String address =
            new NumberMigrator(TextSecurePreferences.getLocalNumber(context))
                .migrate(cursor.getString(0));
        ContentValues contentValues = new ContentValues(1);

        contentValues.put("registered", cursor.getInt(1) == 1 ? 1 : 2);

        if (db.update(
                "recipient_preferences", contentValues, "recipient_ids = ?", new String[] {address})
            < 1) {
    ...
    if (oldVersion < UNREAD_COUNT_VERSION) {
      ...
      try (Cursor cursor =
          db.query("thread", new String[] {"_id"}, "read = 0", null, null, null, null)) {
        while (cursor != null && cursor.moveToNext()) {
          long threadId = cursor.getLong(0);
          int unreadCount = 0;

          try (Cursor smsCursor =
              db.rawQuery(
          ...
          db.execSQL(
              ...
              new String[] {String.valueOf(unreadCount), String.valueOf(threadId)});
        }
      }
    }
    ...
    if (oldVersion < MORE_RECIPIENT_FIELDS) {
      ...
      if (Permissions.hasAny(
        ...
        try (Cursor cursor =
            db.query("recipient_preferences", null, null, null, null, null, null)) {
          while (cursor != null && cursor.moveToNext()) {
            Address address =
            ...
            if (address.isPhone() && !TextUtils.isEmpty(address.toPhoneString())) {
              Uri lookup =
                  Uri.withAppendedPath(
                      ...
                      Uri.encode(address.toPhoneString()));
                ...
                if (contactCursor != null && contactCursor.moveToFirst()) {
                  ...
                  contentValues.put(
                      ...
                      ContactsContract.Contacts.getLookupUri(
                              contactCursor.getLong(2), contactCursor.getString(1))
                          .toString());
                  ...
                  db.update(
                      ...
                      contentValues,
                      "recipient_ids = ?",
                      new String[] {address.toPhoneString()});
                }
              }
            }
          }
  ...
  private static class PostCanonicalAddressIdentityMismatchList {
    ...
    public PostCanonicalAddressIdentityMismatchList(
        List<PostCanonicalAddressIdentityMismatchDocument> list) {
      this.list = list;
    }
  }
  ...
  private static class PostCanonicalAddressNetworkFailureList {
    ...
    public PostCanonicalAddressNetworkFailureList(
        List<PostCanonicalAddressNetworkFailureDocument> list) {
      this.list = list;
    }
  }
  ...
  private static class NumberMigrator {
    ...
    private static final Set<String> SHORT_COUNTRIES =
        new HashSet<String>() {
          {
            add("NU");
            add("TK");
    ...
    public String migrate(@Nullable String number) {
      ...
      if (bareNumber.length() <= 6
          && ("DE".equals(localCountryCode)
              || "FI".equals(localCountryCode)
              || "SK".equals(localCountryCode))) {
        return bareNumber;
      }

      // libphonenumber seems incorrect for Russia and a few other countries with 4 digit short
      // codes.
      ...
      try {
        ...
        if (ShortNumberInfo.getInstance()
            .isPossibleShortNumberForRegion(parsedNumber, localCountryCode)) {
          return bareNumber;
        }
        ...
        if (localNumberImprecise.length() == bareNumber.length()
            || bareNumber.length() > localNumberImprecise.length()) return "+" + number;
  ...
  private static class OldDirectoryDatabaseHelper extends SQLiteOpenHelper {
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
      if (oldVersion < INTRODUCED_CHANGE_FROM_TOKEN_TO_E164_NUMBER) {
        ...
        db.execSQL(
            "CREATE TABLE directory ( _id INTEGER PRIMARY KEY, "
                + "number TEXT UNIQUE, "
                + "registered INTEGER, "
                + "relay TEXT, "
