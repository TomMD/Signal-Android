...
public class RecipientDatabase extends Database {
  ...
  static final List<String> TYPED_RECIPIENT_PROJECTION =
      Stream.of(RECIPIENT_PROJECTION).map(columnName -> TABLE_NAME + "." + columnName).toList();
  ...
  public Cursor getBlocked() {
    ...
    return database.query(
        TABLE_NAME, new String[] {ID, ADDRESS}, BLOCK + " = 1", null, null, null, null, null);
  }
  ...
  Optional<RecipientSettings> getRecipientSettings(@NonNull Cursor cursor) {
    ...
    boolean seenInviteReminder =
        cursor.getInt(cursor.getColumnIndexOrThrow(SEEN_INVITE_REMINDER)) == 1;
    int defaultSubscriptionId =
        cursor.getInt(cursor.getColumnIndexOrThrow(DEFAULT_SUBSCRIPTION_ID));
    ...
    String signalProfileAvatar =
        cursor.getString(cursor.getColumnIndexOrThrow(SIGNAL_PROFILE_AVATAR));
  ...
  public void setSeenInviteReminder(
      @NonNull Recipient recipient, @SuppressWarnings("SameParameterValue") boolean seen) {
  ...
  public void setProfileSharing(
      @NonNull Recipient recipient, @SuppressWarnings("SameParameterValue") boolean enabled) {
    ContentValues contentValues = new ContentValues(1);
  ...
  public Set<Address> getAllAddresses() {
    ...
    try (Cursor cursor =
        db.query(TABLE_NAME, new String[] {ADDRESS}, null, null, null, null, null)) {
  ...
  public void setRegistered(
      @NonNull List<Address> activeAddresses, @NonNull List<Address> inactiveAddresses) {
    for (Address activeAddress : activeAddresses) {
      ContentValues contentValues = new ContentValues(1);
      contentValues.put(REGISTERED, RegisteredState.REGISTERED.getId());

      updateOrInsert(activeAddress, contentValues);
      Recipient.applyCached(
          activeAddress, recipient -> recipient.setRegistered(RegisteredState.REGISTERED));
    }
    ...
    for (Address inactiveAddress : inactiveAddresses) {
      ...
      Recipient.applyCached(
          inactiveAddress, recipient -> recipient.setRegistered(RegisteredState.NOT_REGISTERED));
    }
  }
  ...
  public List<Address> getSystemContacts() {
    ...
    try (Cursor cursor =
        db.query(
            TABLE_NAME,
            new String[] {ADDRESS},
            SYSTEM_DISPLAY_NAME + " IS NOT NULL AND " + SYSTEM_DISPLAY_NAME + " != \"\"",
            null,
            null,
            null,
            null)) {
  ...
  private void updateOrInsert(Address address, ContentValues contentValues) {
    ...
    int updated =
        database.update(
            TABLE_NAME, contentValues, ADDRESS + " = ?", new String[] {address.serialize()});
  ...
  public class BulkOperationsHandle {
    ...
    public void setSystemContactInfo(
      ...
      pendingContactInfoMap.put(
          address,
          new PendingContactInfo(displayName, photoUri, systemPhoneLabel, systemContactUri));
    }
  ...
  private static class PendingContactInfo {
    ...
    private PendingContactInfo(
        String displayName, String photoUri, String phoneLabel, String contactUri) {
