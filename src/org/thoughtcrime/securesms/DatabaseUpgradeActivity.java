  ...
  public void onCreate(Bundle bundle) {
    ...
    if (needsUpgradeTask()) {
      ...
      new DatabaseUpgradeTask(indeterminateProgress, determinateProgress)
          .executeOnExecutor(
              AsyncTask.THREAD_POOL_EXECUTOR, VersionTracker.getLastSeenVersion(this));
    } else {
  ...
  private class DatabaseUpgradeTask extends AsyncTask<Integer, Double, Void>
    ...
    protected Void doInBackground(Integer... params) {
      ...
      if (params[0] < SCREENSHOTS) {
        boolean screenSecurity =
            PreferenceManager.getDefaultSharedPreferences(context)
                .getBoolean(TextSecurePreferences.SCREEN_SECURITY_PREF, true);
    ...
    private void schedulePendingIncomingParts(Context context) {
      ...
      final List<DatabaseAttachment> pendingAttachments =
          DatabaseFactory.getAttachmentDatabase(context).getPendingAttachments();
      ...
      for (DatabaseAttachment attachment : pendingAttachments) {
        ...
        if (attachment.hasData()) {
          Log.w(
              TAG,
              "corrected a pending media part "
                  + attachment.getAttachmentId()
                  + "that already had data.");
          attachmentDb.setTransferState(
              attachment.getMmsId(),
              attachment.getAttachmentId(),
          ...
          Log.w(
              TAG,
              "queuing new attachment download job for incoming push part "
                  + attachment.getAttachmentId()
                  + ".");
          ApplicationContext.getInstance(context)
              ...
              .add(
                  new AttachmentDownloadJob(
                      context, attachment.getMmsId(), attachment.getAttachmentId(), false));
        }
        reader.close();
      }
    }
    ...
    private void scheduleMessagesInPushDatabase(Context context) {
      ...
      try {
        ...
        if (pushReader != null) pushReader.close();
      }
    }
