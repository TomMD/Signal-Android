...
public class IdentityKeyUtil {
  ...
  private static final String IDENTITY_PUBLIC_KEY_CIPHERTEXT_LEGACY_PREF =
      "pref_identity_public_curve25519";
  private static final String IDENTITY_PRIVATE_KEY_CIPHERTEXT_LEGACY_PREF =
      "pref_identity_private_curve25519";
  ...
  public static boolean hasIdentityKey(Context context) {
    SharedPreferences preferences =
        context.getSharedPreferences(MasterSecretUtil.PREFERENCES_NAME, 0);
  ...
  public static @NonNull IdentityKeyPair getIdentityKeyPair(@NonNull Context context) {
    ...
    try {
      ...
      ECPrivateKey privateKey =
          Curve.decodePrivatePoint(Base64.decode(retrieve(context, IDENTITY_PRIVATE_KEY_PREF)));
  ...
  public static void migrateIdentityKeys(
      @NonNull Context context, @NonNull MasterSecret masterSecret) {
    if (!hasIdentityKey(context)) {
      if (hasLegacyIdentityKeys(context)) {
        ...
        save(
            context,
            IDENTITY_PUBLIC_KEY_PREF,
            Base64.encodeBytes(legacyPair.getPublicKey().serialize()));
        save(
            context,
            IDENTITY_PRIVATE_KEY_PREF,
            Base64.encodeBytes(legacyPair.getPrivateKey().serialize()));
  ...
  public static List<BackupProtos.SharedPreference> getBackupRecord(@NonNull Context context) {
    ...
    return new LinkedList<BackupProtos.SharedPreference>() {
      {
        add(
            BackupProtos.SharedPreference.newBuilder()
  ...
  private static boolean hasLegacyIdentityKeys(Context context) {
    return retrieve(context, IDENTITY_PUBLIC_KEY_CIPHERTEXT_LEGACY_PREF) != null
        && retrieve(context, IDENTITY_PRIVATE_KEY_CIPHERTEXT_LEGACY_PREF) != null;
  }

  private static IdentityKeyPair getLegacyIdentityKeyPair(
      @NonNull Context context, @NonNull MasterSecret masterSecret) {
    try {
      MasterCipher masterCipher = new MasterCipher(masterSecret);
      byte[] publicKeyBytes =
          Base64.decode(retrieve(context, IDENTITY_PUBLIC_KEY_CIPHERTEXT_LEGACY_PREF));
      IdentityKey identityKey = new IdentityKey(publicKeyBytes, 0);
      ECPrivateKey privateKey =
          masterCipher.decryptKey(
              Base64.decode(retrieve(context, IDENTITY_PRIVATE_KEY_CIPHERTEXT_LEGACY_PREF)));
