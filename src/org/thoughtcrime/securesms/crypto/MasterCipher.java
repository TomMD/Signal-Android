 ...
 *
 * <p>The protocol format is roughly:
 *
 * <p>1) 16 byte random IV. 2) AES-CBC(plaintext) 3) HMAC-SHA1 of 1 and 2
 *
...
public class MasterCipher {
  ...
  private byte[] getDecryptedBody(Cipher cipher, byte[] encryptedBody)
      throws IllegalBlockSizeException, BadPaddingException {
    return cipher.doFinal(
        encryptedBody, cipher.getBlockSize(), encryptedBody.length - cipher.getBlockSize());
  }
  ...
  private Cipher getDecryptingCipher(SecretKeySpec key, byte[] encryptedBody)
  ...
  private Cipher getEncryptingCipher(SecretKeySpec key)
