/**
 * Copyright (C) 2011 Whisper Systems Copyright (C) 2013 Open Whisper Systems
 *
...
/**
 * This class is used to asymmetricly encrypt local data. This is used in the case where TextSecure
 * receives an SMS, but the user's local encryption passphrase is not cached (either because of a
 * timeout, or because it hasn't yet been entered).
 *
 * <p>In this case, we have access to the public key of a local keypair. We encrypt the message with
 * this, and put it into the DB. When the user enters their passphrase, we can get access to the
 * private key of the local keypair, decrypt the message, and replace it into the DB with symmetric
 * encryption.
 *
 * <p>The encryption protocol is as follows:
 *
 * <p>1) Generate an ephemeral keypair. 2) Do ECDH with the public key of the local durable keypair.
 * 3) Do KMF with the ECDH result to obtain a master secret. 4) Encrypt the message with that master
 * secret.
 *
