...
public class ClassicDecryptingPartInputStream {
  ...
  public static InputStream createFor(
      @NonNull AttachmentSecret attachmentSecret, @NonNull File file) throws IOException {
    try {
      ...
      return new CipherInputStreamWrapper(
          new LimitedInputStream(fileStream, file.length() - MAC_LENGTH - IV_LENGTH), cipher);
  ...
  private static void verifyMac(AttachmentSecret attachmentSecret, File file) throws IOException {
    ...
    InputStream dataStream =
        new LimitedInputStream(new FileInputStream(file), file.length() - MAC_LENGTH);
  ...
  private static class CipherInputStreamWrapper extends CipherInputStream {
    ...
    public long skip(long skipAmount) throws IOException {
      ...
      while (remaining > 0) {
        int read =
            super.read(skipBuffer, 0, Util.toIntExact(Math.min(skipBuffer.length, remaining)));
