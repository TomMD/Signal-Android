 ...
 *
 * <p>On modern Android, the serialized secrets are themselves encrypted using a key that lives in
 * the system KeyStore, for whatever that is worth.
 */
public class AttachmentSecretProvider {
  ...
  public synchronized AttachmentSecret setClassicKey(
      @NonNull Context context, @NonNull byte[] classicCipherKey, @NonNull byte[] classicMacKey) {
  ...
  private AttachmentSecret getUnencryptedAttachmentSecret(
    ...
    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
      ...
      KeyStoreHelper.SealedData encryptedSecret =
          KeyStoreHelper.seal(attachmentSecret.serialize().getBytes());
  ...
  private void storeAttachmentSecret(
      @NonNull Context context, @NonNull AttachmentSecret attachmentSecret) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
      KeyStoreHelper.SealedData encryptedSecret =
          KeyStoreHelper.seal(attachmentSecret.serialize().getBytes());
      TextSecurePreferences.setAttachmentEncryptedSecret(context, encryptedSecret.serialize());
