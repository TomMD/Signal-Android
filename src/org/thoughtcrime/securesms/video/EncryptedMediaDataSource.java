...
public class EncryptedMediaDataSource extends MediaDataSource {
  ...
  private int readAtClassic(long position, byte[] bytes, int offset, int length)
      throws IOException {
    InputStream inputStream =
        ClassicDecryptingPartInputStream.createFor(attachmentSecret, mediaFile);
    ...
    while (headerRemaining > 0) {
      int read =
          inputStream.read(
              buffer, 0, Util.toIntExact(Math.min((long) buffer.length, headerRemaining)));
  ...
  private int readAtModern(long position, byte[] bytes, int offset, int length) throws IOException {
    ...
    InputStream inputStream =
        ModernDecryptingPartInputStream.createFor(attachmentSecret, random, mediaFile, position);
    int returnValue = inputStream.read(bytes, offset, length);
