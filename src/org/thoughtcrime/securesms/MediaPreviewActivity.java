...
public class MediaPreviewActivity extends PassphraseRequiredActionBarActivity
    implements RecipientModifiedListener, LoaderManager.LoaderCallbacks<Pair<Cursor, Integer>> {
      ...
      if (mediaItem.date > 0) {
        relativeTimeSpan =
            DateUtils.getExtendedRelativeTimeSpanString(
                this, dynamicLanguage.getCurrentLocale(), mediaItem.date);
      } else {
          ...
          (dialogInterface, i) -> {
            Permissions.with(this)
                ...
                .withPermanentDenialDialog(
                    getString(
                        R.string
                            .MediaPreviewActivity_signal_needs_the_storage_permission_in_order_to_write_to_external_storage_but_it_has_been_permanently_denied))
                .onAnyDenied(
                    () ->
                        Toast.makeText(
                                this,
                ...
                .onAllGranted(
                    () -> {
                      SaveAttachmentTask saveTask =
                          new SaveAttachmentTask(MediaPreviewActivity.this);
                      long saveDate =
                          (mediaItem.date > 0) ? mediaItem.date : System.currentTimeMillis();
        ...
        (dialogInterface, which) -> {
          new AsyncTask<Void, Void, Void>() {
            ...
            protected Void doInBackground(Void... voids) {
              ...
              AttachmentUtil.deleteAttachment(
                  MediaPreviewActivity.this.getApplicationContext(), mediaItem.attachment);
              return null;
            }
          }.execute();
  ...
  public void onLoadFinished(
      Loader<Pair<Cursor, Integer>> loader, @Nullable Pair<Cursor, Integer> data) {
    if (data != null) {
      @SuppressWarnings("ConstantConditions")
      CursorPagerAdapter adapter =
          new CursorPagerAdapter(
              this, GlideApp.with(this), getWindow(), data.first, data.second, leftIsRecent);
  ...
  private static class CursorPagerAdapter extends PagerAdapter implements MediaItemAdapter {
    ...
    public MediaItem getMediaItemFor(int position) {
      ...
      return new MediaItem(
          address != null ? Recipient.from(context, address, true) : null,
