/**
 * * Copyright (c) 2013-2014 CommonsWare, LLC Portions Copyright (C) 2007 The Android Open Source
 * Project
 *
...
public class CameraView extends ViewGroup {
  ...
  private @NonNull List<CameraViewListener> listeners =
      Collections.synchronizedList(new LinkedList<CameraViewListener>());
  ...
  public void onResume() {
    ...
    enqueueTask(
        new SerialAsyncTask<Void>() {
          @Override
          protected @Nullable Void onRunBackground() {
            try {
              long openStartMillis = System.currentTimeMillis();
              camera = Optional.fromNullable(Camera.open(cameraId));
              Log.w(
                  TAG, "camera.open() -> " + (System.currentTimeMillis() - openStartMillis) + "ms");
  ...
  public void setPreviewCallback(final @NonNull PreviewCallback previewCallback) {
    enqueueTask(
        new PostInitializationTask<Void>() {
          @Override
          protected void onPostMain(Void avoid) {
            if (camera.isPresent()) {
              camera
                        ...
                        public void onPreviewFrame(byte[] data, Camera camera) {
                          ...
                          if (data != null) {
                            previewCallback.onPreviewFrame(
                                new PreviewFrame(
                                    data, previewSize.width, previewSize.height, rotation));
                          }
                        }
                      });
            }
    ...
    if (this.camera.isPresent()) {
      try {
        ...
        if (preferredPreviewSize != null
            && !parameters.getPreviewSize().equals(preferredPreviewSize)) {
          Log.w(
              TAG,
              "starting preview with size "
                  + preferredPreviewSize.width
                  + "x"
        ...
        Log.w(
            TAG,
            "camera.startPreview() -> " + (System.currentTimeMillis() - previewStartMillis) + "ms");
        state = State.ACTIVE;
        Util.runOnMain(
            new Runnable() {
              @Override
  ...
  private Size getPreferredPreviewSize(@NonNull Parameters parameters) {
    return CameraUtils.getPreferredPreviewSize(
        displayOrientation, getMeasuredWidth(), getMeasuredHeight(), parameters);
  }
        ...
        .setOneShotPreviewCallback(
            new Camera.PreviewCallback() {
              ...
              public void onPreviewFrame(byte[] data, final Camera camera) {
                ...
                new CaptureTask(previewSize, rotation, croppingRect)
  ...
  private Rect getCroppedRect(Size cameraPreviewSize, Rect visibleRect, int rotation) {
    ...
    final float centerX =
        (VERSION.SDK_INT < 14 || isTroublemaker()) ? previewWidth - newWidth / 2 : previewWidth / 2;
  ...
  private abstract static class SerialAsyncTask<Result> extends Job {
  ...
  private class CaptureTask extends AsyncTask<byte[], Void, byte[]> {
    ...
    protected byte[] doInBackground(byte[]... params) {
      ...
      try {
        return BitmapUtil.createFromNV21(
            data,
            previewSize.width,
            previewSize.height,
            rotation,
