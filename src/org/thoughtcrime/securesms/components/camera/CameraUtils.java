...
public class CameraUtils {
   ...
   */
  public static @Nullable Size getPreferredPreviewSize(
      int displayOrientation, int width, int height, @NonNull Parameters parameters) {
    ...
    Log.w(
        ...
        String.format(
            ...
            displayOrientation, width, height, targetWidth, targetHeight, targetRatio));
    ...
    for (Size size : sizes) {
      Log.w(
          TAG,
          String.format(
              "  %dx%d (%.02f)", size.width, size.height, (float) size.width / size.height));

      if (size.height == size.width * targetRatio
  ...
  public static int getCameraDisplayOrientation(
      @NonNull Activity activity, @NonNull CameraInfo info) {
