...
public class EmojiPageView extends FrameLayout {
  ...
  public EmojiPageView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    final View view =
        LayoutInflater.from(getContext()).inflate(R.layout.emoji_grid_layout, this, true);
    grid = (GridView) view.findViewById(R.id.emoji);
    grid.setColumnWidth(
        getResources().getDimensionPixelSize(R.dimen.emoji_drawer_size)
            + 2 * getResources().getDimensionPixelSize(R.dimen.emoji_drawer_item_padding));
  ...
  private static class EmojiGridAdapter extends BaseAdapter {
    ...
    public View getView(final int position, final View convertView, final ViewGroup parent) {
      ...
      final int pad =
          context.getResources().getDimensionPixelSize(R.dimen.emoji_drawer_item_padding);
      if (convertView != null && convertView instanceof EmojiView) {
        ...
        emojiView.setLayoutParams(
            new AbsListView.LayoutParams(emojiSize + 2 * pad, emojiSize + 2 * pad));
        view = emojiView;
      }
