...
public class ContactsDatabase {
  ...
  private static final String CONTACT_MIMETYPE =
  ...
  public synchronized void setRegisteredUsers(
      @NonNull Account account, @NonNull List<Address> registeredAddressList, boolean remove)
      ...
      if (!currentContacts.containsKey(registeredAddress)) {
        ...
        if (systemContactInfo.isPresent()) {
          ...
          addTextSecureRawContact(
              operations,
              account,
              systemContactInfo.get().number,
              systemContactInfo.get().name,
              systemContactInfo.get().id,
              true);
        }
      }
    }
    ...
    for (Map.Entry<Address, SignalContact> currentContactEntry : currentContacts.entrySet()) {
      if (!registeredAddressSet.contains(currentContactEntry.getKey())) {
        ...
        addContactVoiceSupport(
            operations, currentContactEntry.getKey(), currentContactEntry.getValue().getId());
      } else if (!Util.isStringEquals(
          currentContactEntry.getValue().getRawDisplayName(),
          currentContactEntry.getValue().getAggregateDisplayName())) {
        Log.w(TAG, "Updating display name: " + currentContactEntry.getKey());
        updateDisplayName(
            operations,
            currentContactEntry.getValue().getAggregateDisplayName(),
            currentContactEntry.getValue().getId(),
            currentContactEntry.getValue().getDisplayNameSource());
      }
    }
  ...
  Cursor querySystemContacts(@Nullable String filter) {
    ...
    if (!TextUtils.isEmpty(filter)) {
      uri =
          Uri.withAppendedPath(
              ContactsContract.CommonDataKinds.Phone.CONTENT_FILTER_URI, Uri.encode(filter));
    } else {
    ...
    String excludeSelection =
        "("
            + formattedNumber
            + " NOT IN "
            + "(SELECT data1 FROM view_data WHERE "
            + formattedNumber
            + " = data1) "
            + "OR "
            + formattedNumber
    ...
    return new ProjectionMappingCursor(
        cursor, projectionMap, new Pair<String, Object>(CONTACT_TYPE_COLUMN, NORMAL_TYPE));
  }

  @NonNull
  Cursor queryTextSecureContacts(String filter) {
    String[] projection =
        new String[] {ContactsContract.Contacts.DISPLAY_NAME, ContactsContract.Data.DATA1};
          ...
          context
              ...
              .query(
                  ...
                  ContactsContract.Data.MIMETYPE
                      ...
                      + " LIKE ?)",
                  new String[] {CONTACT_MIMETYPE, "%" + filter + "%", "%" + filter + "%"},
                  sort);
    }
  ...
  private void addContactVoiceSupport(
      List<ContentProviderOperation> operations, @NonNull Address address, long rawContactId) {
    operations.add(
    ...
    operations.add(
        ContentProviderOperation.newInsert(
            ...
            .withValue(
                ...
                context.getString(R.string.ContactsDatabase_signal_call_s, address.toPhoneString()))
    ...
    if (displayNameSource != ContactsContract.DisplayNameSources.STRUCTURED_NAME) {
      operations.add(
          ContentProviderOperation.newInsert(dataUri)
              .withValue(
                  ContactsContract.CommonDataKinds.StructuredName.RAW_CONTACT_ID, rawContactId)
  ...
  private void removeContactVoiceSupport(
      List<ContentProviderOperation> operations, long rawContactId) {
    operations.add(
        ContentProviderOperation.newUpdate(RawContacts.CONTENT_URI)
            .withSelection(RawContacts._ID + " = ?", new String[] {String.valueOf(rawContactId)})
  ...
  private void addTextSecureRawContact(
      List<ContentProviderOperation> operations,
      Account account,
      String e164number,
      String displayName,
    ...
    operations.add(
        ContentProviderOperation.newInsert(dataUri)
            .withValueBackReference(
                ContactsContract.CommonDataKinds.StructuredName.RAW_CONTACT_ID, index)
            .withValue(ContactsContract.CommonDataKinds.StructuredName.DISPLAY_NAME, displayName)
            .withValue(
                ContactsContract.Data.MIMETYPE,
                ContactsContract.CommonDataKinds.StructuredName.CONTENT_ITEM_TYPE)
    ...
    operations.add(
        ContentProviderOperation.newInsert(dataUri)
            ...
            .withValue(
                ContactsContract.Data.DATA3,
                context.getString(R.string.ContactsDatabase_message_s, e164number))
            .withYieldAllowed(true)
            .build());
    ...
    if (supportsVoice) {
      operations.add(
          ContentProviderOperation.newInsert(dataUri)
              ...
              .withValue(
                  ContactsContract.Data.DATA3,
                  context.getString(R.string.ContactsDatabase_signal_call_s, e164number))
              .withYieldAllowed(true)
              .build());
    }
  ...
  private void removeTextSecureRawContact(
      List<ContentProviderOperation> operations, Account account, long rowId) {
    operations.add(
        ContentProviderOperation.newDelete(
  ...
  private @NonNull Map<Address, SignalContact> getSignalRawContacts(@NonNull Account account) {
    ...
    try {
      ...
      if (Build.VERSION.SDK_INT >= 11) {
        ...
        projection =
            new String[] {
              BaseColumns._ID, RawContacts.SYNC1, RawContacts.SYNC4, RawContacts.CONTACT_ID
            };
      }
      ...
      while (cursor != null && cursor.moveToNext()) {
        ...
        signalContacts.put(
            ...
            new SignalContact(
                rawContactId,
                supportsVoice,
                rawContactDisplayName,
                aggregateDisplayName,
                rawContactDisplayNameSource));
      }
    } finally {
  ...
  private Optional<SystemContactInfo> getSystemContactInfo(@NonNull Address address) {
    ...
    Uri uri =
        Uri.withAppendedPath(
            ContactsContract.PhoneLookup.CONTENT_FILTER_URI, Uri.encode(address.toPhoneString()));
    String[] projection = {
      ...
      while (numberCursor != null && numberCursor.moveToNext()) {
        ...
        if (systemAddress.equals(address)) {
          ...
          if (idCursor != null && idCursor.moveToNext()) {
            return Optional.of(
                new SystemContactInfo(
                    numberCursor.getString(2), numberCursor.getString(0), idCursor.getLong(0)));
          }
        }
      }
    } finally {
  ...
  private static class ProjectionMappingCursor extends CursorWrapper {
    ...
    ProjectionMappingCursor(
        Cursor cursor, Map<String, String> projectionMap, Pair<String, Object>... extras) {
      super(cursor);
