...
public class RecipientsEditor extends AppCompatMultiAutoCompleteTextView {
  ...
  public boolean hasValidRecipient(boolean isMms) {
    for (String number : mTokenizer.getNumbers()) {
      if (isValidAddress(number, isMms)) return true;
    }
    return false;
  }
  ...
  public static CharSequence contactToToken(Recipient c) {
    ...
    s.setSpan(
        new Annotation("number", c.getAddress().serialize()),
        0,
        len,
  ...
  private class RecipientsEditorTokenizer implements MultiAutoCompleteTextView.Tokenizer {
    ...
    /**
     * Returns the start of the token that ends at offset <code>cursor</code> within <code>text
     * </code>. It is a method from the MultiAutoCompleteTextView.Tokenizer interface.
     */
     ...
     * </code> within <code>text</code>. It is a method from the MultiAutoCompleteTextView.Tokenizer
     * interface.
     */
     ...
     * terminator (for example a space or comma). It is a method from the
