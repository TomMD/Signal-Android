...
public class ExpirationDialog extends AlertDialog {
  ...
  public static void show(
      final Context context, final int currentExpiration, final @NonNull OnClickListener listener) {
    ...
    builder.setPositiveButton(
        ...
        (dialog, which) -> {
          int selected =
              ((NumberPickerView) view.findViewById(R.id.expiration_number_picker)).getValue();
  ...
  private static View createNumberPickerView(final Context context, final int currentExpiration) {
    ...
    for (int i = 0; i < expirationTimes.length; i++) {
      expirationDisplayValues[i] =
          ExpirationUtil.getExpirationDisplayValue(context, expirationTimes[i]);
    ...
    NumberPickerView.OnValueChangeListener listener =
        (picker, oldVal, newVal) -> {
          if (newVal == 0) {
            ...
            textView.setText(
                context.getString(
                    R.string
                        .ExpirationDialog_your_messages_will_disappear_s_after_they_have_been_seen,
                    picker.getDisplayedValues()[newVal]));
          }
        };
