...
public class AttachmentDatabaseTest extends TextSecureTestCase {
  ...
  public void testThumbnailGenerationTaskNotRunWhenThumbnailExists() throws Exception {
    ...
    doReturn(mockInputStream)
        .when(database)
        .getDataStream(any(MasterSecret.class), any(AttachmentId.class), eq("thumbnail"));
    database.getThumbnailStream(mock(MasterSecret.class), attachmentId);

    // Works as the Future#get() call in AttachmentDatabase#getThumbnailStream() makes updating
    // synchronous
    verify(database, never())
        .updateAttachmentThumbnail(
            any(MasterSecret.class), any(AttachmentId.class), any(InputStream.class), anyFloat());
  }
    ...
    try {
      ...
      if (!(bde.getCause() instanceof FileNotFoundException)) {
        throw new AssertionError(
            "Thumbnail generation failed for another reason than a FileNotFoundException: "
                + bde.getMessage());
      } // else success
    }
  }
